...
new_predictor_normal <- function(mean, sd) {
  new_predictor(f = function(x0) dnorm(x0, mean = mean, sd = sd),
                predictor_type = "continuous", lb = -Inf, ub = Inf)
}
...
new_predictor_snp <- function(maf) {
  new_predictor(f = function(x0) dbinom(x0, size = 2, prob = maf),
...
new_predictor_binary <- function(p) {
  new_predictor(f = function(x0) dbinom(x0, size = 1, prob = p),
